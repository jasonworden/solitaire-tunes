var loudSoundEvent = document.createEvent("HTMLEvents");
loudSoundEvent.initEvent(LOUD_SOUND_EVENT_NAME, true, true);
loudSoundEvent.eventName = LOUD_SOUND_EVENT_NAME;
...
function loadSound() {
	var audioFileUrl = 'vin-sol-cookies.ogg';
...
function updateSound() {
  ...
  // analyzer.getByteFrequencyData(frequencyData);
	...
	var index = Math.floor(analyzer.frequencyBinCount/4);
  var level = frequencyData[index];
  // console.log(level);
  if(level > 205) {
    loudSoundEvent.multiplier = 6;
    document.dispatchEvent(loudSoundEvent);
  } else if(level > 160) {
    loudSoundEvent.multiplier = 1;
    document.dispatchEvent(loudSoundEvent);
  }
}
...
// Three.js graphics based on Mr. Doob's solitaire sketch
...
canvas.setAttribute('id', 'three-js-canvas');
...
var Particle = function ( id, x, y, sx, sy ) {
	...
	this.update = function (delta) {
		...
		sy += 0.98;
		context.drawImage(cardImg, cx, cy, cwidth, cheight,
...
document.addEventListener(window.LOUD_SOUND_EVENT_NAME, function(event) {
  var w = Math.max(document.documentElement.clientWidth, window.innerWidth || 0);
  var h = Math.max(document.documentElement.clientHeight, window.innerHeight || 0);
  var iMax = event.multiplier;
  // console.log(event.multiplier);

  for(var i=0; i<iMax; ++i) {
    var x = Math.floor( Math.random() * w );
		var maxH = event.multiplier === 2 ? h : h/2;
...
function updateSolitaire() {
	requestAnimationFrame(updateSolitaire);
